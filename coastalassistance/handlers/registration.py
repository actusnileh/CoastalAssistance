...
from database.repositories.user import add_user, check_exist_user, update_username
...
async def start(message: Message, state: FSMContext):
    ...
    if await check_exist_user(user_id):
        ...
        try:
            ...
            await update_username(username, user_id)
...
async def registration_confirmation(message: Message, state: FSMContext):
    ...
    if re.search(r"да", answer, re.IGNORECASE):
        try:
            ...
            await update_username(user_name, user_id)
            await state.clear()
            await message.answer(
                SUCCESSFUL_REGISTER.format(first_name=user_first_name),
                reply_markup=await main_menu(),
